...
save_file       = 'mse_spring_constants_rad.csv'    # File to save MSE values into
device          = 'cuda'                            # Device to run inference on
molecules_dir   = '../../molecules'                 # Path to molecule database
sim_save_dir    = './test_sim_rad'                  # Directory where to save test simulations for inspection
    ...
    for ib, batch in enumerate(trainer):
        ...
        if ib == 0:
            ...
            for s in range(10):
                ...
                plt.savefig(os.path.join(sim_save_dir, f'sim{s}_krad_{k_rad:.3f}.png'))
                plt.close()
