...
width_ratios = [6, 4, 2]
gap = 0.15
fig = plt.figure(figsize=(sum(width_ratios), 8.5))
fig_grid = fig.add_gridspec(1, len(width_ratios), wspace=0.02, hspace=0, width_ratios=width_ratios)
afm_grid = fig_grid[0, 0].subgridspec(2, 1, wspace=0, hspace=gap)
pred_grid = fig_grid[0, 1].subgridspec(2, 1, wspace=0, hspace=gap)
...
data_lims_hm = (
    ...
    max(pred_sim[1].max(), pred_exp[1].max())
)
...
plt.savefig('bcb.pdf', bbox_inches='tight')
