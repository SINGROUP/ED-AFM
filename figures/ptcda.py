...
Y_hartree = np.load(os.path.join(data_dir, 'PTCDA/ESMapHartree.npy'))

# Create figure grid
fig_width = 0.1/2.54*fig_width
width_ratios = [6, 8, 0.3]
height_ratios = [1, 0.778]
gap = 0.25
fig = plt.figure(figsize=(fig_width, 5.75*fig_width/sum(width_ratios)))
...
vmax = max(
    ...
    abs(Y_hartree.min()), abs(Y_hartree.max())
)
vmin = -vmax
...
xyz_img = np.flipud(imageio.imread(os.path.join(data_dir, 'PTCDA/mol.png')))
...
# Set titles
afm_sim_axes[0, len(X_slices)//2].set_title('AFM simulation', fontsize=fontsize, y=0.90)
afm_exp_axes[0, len(X_slices)//2].set_title('AFM experiment', fontsize=fontsize, y=0.90)
pred_sim_ax.set_title('Sim. prediction', fontsize=fontsize, y=0.95)
pred_exp_ax.set_title('Exp. prediction', fontsize=fontsize, y=0.94)
ref_pc_ax.set_title('Reference', fontsize=fontsize, y=0.95)

plt.savefig('ptcda.pdf', bbox_inches='tight', dpi=dpi)
