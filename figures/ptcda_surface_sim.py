...
# Load data
X1 = np.load(os.path.join(data_dir, 'PTCDA/data_CO_sim.npz'))['data'].astype(np.float32)
X2 = np.load(os.path.join(data_dir, 'PTCDA/data_Xe_sim.npz'))['data'].astype(np.float32)
X = [X1[None], X2[None]]
Y_hartree = [np.load(os.path.join(data_dir, 'PTCDA/ESMapHartree.npy'))[None]]
