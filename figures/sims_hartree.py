...
for ib in range(len(molecules)):
    ...
    # Load data
    X1 = np.load(os.path.join(molecules[ib], 'data_CO.npz'))['data'].astype(np.float32)
    X2 = np.load(os.path.join(molecules[ib], 'data_Xe.npz'))['data'].astype(np.float32)
    X = [X1[None], X2[None]]
    ...
    rel_abs_err_es = np.mean(np.abs(pred[0] - Y_hartree[0])) / np.ptp(Y_hartree[0])
    print(f'Relative error: {rel_abs_err_es*100:.2f}%')

plt.savefig('sims_hartree.pdf', bbox_inches='tight', dpi=dpi)
